...
local gossipData = addon:T()

function provider:OnPinClick(pin, button, down)
	if button == 'LeftButton' and not down then
		local gossipOptionID = pin:GetID()
...
local function updatePin(pin, gossipInfo)
	...
	if data.isQuest or (gossipInfo.flags and FlagsUtil.IsSet(gossipInfo.flags, Enum.GossipOptionRecFlags.QuestLabelPrepend)) then
		pin:SetSize(data.size or 32, data.size or 32)
		pin:SetNormalAtlas('quest-campaign-available')
		pin:SetHighlightAtlas('quest-campaign-available', 'ADD')
	elseif data.isTaxi then
		pin:SetSize(20, 20)
		pin:SetNormalAtlas('Taxi_Frame_Gray')
		pin:SetHighlightAtlas('Taxi_Frame_Yellow', 'BLEND')
	else
		pin:SetWidth(data.atlasWidth or data.atlasSize or 24)
		pin:SetHeight(data.atlasHeight or data.atlasSize or 24)
		pin:SetNormalAtlas(data.atlas or 'MagePortalAlliance')
		pin:SetHighlightAtlas(data.highlightAtlas or data.atlas or 'MagePortalHorde', data.highlightAdd and 'ADD' or 'BLEND')
	end
...
function provider:OnRefresh()
	for _, gossipInfo in next, gossipData do
		local mapID, x, y
		if gossipInfo.data.isTaxiSource then
			...
			mapID = gossipInfo.data.mapID
			x = gossipInfo.data.x
			y = gossipInfo.data.y
		end

		if mapID and x and y then
			local pin = self:AddPin(mapID, x, y)
			if pin then
				updatePin(pin, gossipInfo)
			end
		end
	end
end
...
function addon:GOSSIP_SHOW()
	...
	if #gossipData == 0 then
		addon:ResetActiveMaps()
		return
	elseif #gossipData == 1 and addon:GetOption('selectSingle') then
		local gossipOptionID = gossipData[1].gossipOptionID
		local data = addon.data[gossipOptionID]
