local L = select(2, ...).L('esES')

L['Click to travel'] = 'Haga clic para viajar'
-- L['You will end up in one of multiple locations within this zone!'] = '' -- MISSING!
L['Not Discovered'] = TAXI_PATH_UNREACHABLE

-- pulled from LibBabble-SubZone-3.0
L['Galebroken Path'] = 'Sendero Rompeciclón'
L['Thorignir Refuge'] = 'Refugio de los Thorignir'
L['Thorim\'s Peak'] = 'Cima de Thorim'

L['Eastern Kingdoms'] = 'Reinos del Este'
L['Kalimdor'] = 'Kalimdor'
L['Outland'] = 'Terrallende'
L['Northrend'] = 'Rasganorte'
L['Pandaria'] = 'Pandaria'
L['Draenor'] = 'Draenor'
L['Broken Isles'] = 'Islas Abruptas'

L['Ironforge'] = 'Forjaz'
L['Stormwind'] = 'Ventormenta'
L['Nethergarde Keep'] = 'Castillo de Nethergarde'
L['Aerie Peak'] = 'Pico Nidal'
L['The Masonary'] = 'El Masón'
L['Shadowforge City'] = 'Ciudad Forjatiniebla'
L['Fire Plume Ridge'] = 'Cresta del Penacho en Llamas'
L['Throne of Flame'] = 'El Trono de la Llama'
L['The Great Divide'] = 'La Gran División'
L['Fel Pits'] = 'Las Fosas Viles'
L['Honor Hold'] = 'Bastión del Honor'
L['Skald'] = 'Skald'
L['Argent Tournament Grounds'] = 'Campos del Torneo Argenta'
L['Ruby Dragonshrine'] = 'Santuario de Dragones Rubí'
L['One Keg'] = 'Barrilia'
L['Stormstout Brewery'] = 'Cervecería del Trueno'
-- L['Blackrock Foundry Overlook'] = '' -- MISSING TRANSLATION
L['Elemental Plateau'] = 'Meseta Elemental'
L['Neltharion\'s Vault'] = 'Cámara de Neltharion'
L['Broken Shore'] = 'Costa Abrupta'

-- pulled from http://es.wowhead.com/quest=41950
L['Cry More Thunder!'] = '¡Truenos y truenos!'

L['Sewer Guard Station'] = 'Puesto de guardia de cloaca'
L['Black Market'] = 'El Mercado Negro'
L['Inn Entrance'] = 'Entrada de la posada'
L['Alchemists\' Lair'] = 'Guarida de los alquimistas'
L['Abandoned Shack'] = 'Cabaña abandonada'
L['Rear Entrance'] = 'Entrada trasera'
